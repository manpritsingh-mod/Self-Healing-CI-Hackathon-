...
class ConfidenceLoop:
    ...
    async def run(
        ...
        for loop_no in range(1, MAX_LOOPS + 1):
            ...
            # ── Self-Evaluating Fixer Check ──
            if hasattr(fix_result, 'confidence') and getattr(fix_result, 'confidence', 0) >= CONFIDENCE_THRESHOLD:
                logger.info(
                    f"[LOOP] ✅ SELF-RATED HIGH CONFIDENCE — fix agent scored {fix_result.confidence}%. "
                    f"Bypassing Validator to save tokens!"
                )
                attempt = LoopAttempt(
                    loop_no=loop_no,
                    fix_candidate=fix_result.fix_description,
                    validator_feedback="Bypassed (Self-Rated High Confidence)",
