...
class ValidatorAgent(BaseAgent):
    ...
    async def analyze(
        ...
        token_budget.spend(tokens_used)
        ...
        parsed = ai_service.parse_json_response(response["content"])

        if parsed:
            confidence = int(parsed.get("confidence", 0))
            approved = parsed.get("approved", False) and confidence >= 90
            ...
            self.logger.warning("Could not parse JSON from validator response")
            return ValidatorResult(
                approved=False,
