...
class NotifyAgent(BaseAgent):
    ...
    async def analyze(self, incident: Incident):
        ...
        stored = False
        if incident.final_confidence >= CONFIDENCE_THRESHOLD:
            await self._store_incident_memory(incident)
            stored = True
        else:
            self.logger.info(
                f"Skipping incident memory storage: confidence {incident.final_confidence}% "
                f"is below threshold {CONFIDENCE_THRESHOLD}%"
            )

        self.logger.info(
            f"Notification complete: Slack={'✅' if slack_sent else '❌'} "
            f"Email={'✅' if email_sent else '❌'} Memory={'✅' if stored else '⏭️ Skipped'}"
        )
    ...
    async def _store_incident_memory(self, incident: Incident):
        ...
        try:
            ...
            metadata = {
                ...
                "affected_file": incident.root_cause.affected_file if incident.root_cause else "",
                "affected_line": str(incident.root_cause.affected_line) if incident.root_cause and incident.root_cause.affected_line else "",
